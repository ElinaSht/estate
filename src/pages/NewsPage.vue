...
<template>
  ...
  <div class="w-full mb-[52px] flex gap-[15px] justify-center md:max-lg:mb-[42px] max-md:mb-[32px] max-sm:flex-col max-sm:items-center max-sm:gap-[8px]">
    <TheTag label="Все новости и статьи" :active="!filter" @click="filter = undefined" />
    <TheTag label="Новости" :active="filter === PostType.NEWS" @click="filter = PostType.NEWS" />
    <TheTag label="Статьи" :active="filter === PostType.ARTICLE" @click="filter = PostType.ARTICLE" />
  </div>
